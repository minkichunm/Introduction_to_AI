		...
		for (int d = 0; d < depth; d++) {
			...
			for (int i = 0; i < positions.size(); i++) {
				...
				for (int j = 0; j < destinations.size(); j++) {
					...
					int recHeuristic = RecMinMaxMove_v2(board, d , vmc,  (byte) ( ((vmc.getTurnOfPlayer()+2)%2)+1), alpha, beta) - heuristicCutOff; 
...
//			System.out.println("RandomMove in Greedy");
	...
	private int RecMinMaxMove_v2(Board fakeBoard, int depth, ValidMovesCalculator vmc,  byte playersTurn, int alpha, int beta) {
		...
		if (depth == 0) {
			...
			//return Heuristic3(fakeBoard, playersTurn);
	...
	public int Heuristic3(Board board, byte player) { 
		int result1 = 0;
